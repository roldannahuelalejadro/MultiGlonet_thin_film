...
def TMM_solver(thicknesses, refractive_indices, n_bot, n_top, k, theta, pol = 'TM'):
    ...
    Transmision =  1 / torch.pow(complex_abs(S_stack[0]), 2)

            
    return Reflection , Transmision


def MTMM_solver(condiciones, thicknesses, refractive_indices, physicsparams):
    '''
    arg:
    ...
    for i in range(1, condiciones + 1):
        resultado, transmision = TMM_solver(
            thicknesses,
            refractive_indices,
            getattr(physicsparams, f"n_bot_{i}"),
            getattr(physicsparams, f"n_top_{i}"),
            getattr(physicsparams, f"k_{i}"),
            getattr(physicsparams, f"theta_{i}"),
            getattr(physicsparams, f"pol_{i}")
        )
        resultados_reflexion[f"reflexion_{i}"] = resultado
        resultados_transmision[f"transmision_{i}"] = transmision

    return resultados_reflexion, resultados_transmision
