    ...
    def collect_user_input(self):
        if self.user_define:
            for i in range(1, self.condiciones + 1):
                ...
                while True:
                    try:
                        setattr(self, f"pol_{i}", str(input(f"Ingrese el valor para pol_{i} u cualquier otro para ambos: ")))
                        break
                    except ValueError:
    ...
    def generate_physics_params(self):
        if self.user_define:
            for i in range(1, self.condiciones + 1):
                ...
                # Convertir theta a radianes antes de crear el tensor
